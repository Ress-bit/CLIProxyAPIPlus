...
これは [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) の Plus 版であり、本流プロジェクトにサードパーティプロバイダー対応を追加したものです。

サードパーティプロバイダーのサポートはすべてコミュニティメンテナーによって提供されます。CLIProxyAPI 本体では技術サポートを行いません。サポートが必要な場合は、該当するコミュニティメンテナーに連絡してください。

Plus リリースは本流の機能追加に追従します。

OAuth 経由で OpenAI Codex（GPT モデル）および Claude Code もサポートしています。
...
このプロジェクトでは、サードパーティプロバイダー対応に関する Pull Request のみを受け付けています。サードパーティプロバイダー対応と無関係な Pull Request は受け付けられません。

サードパーティプロバイダー対応以外の変更を提案したい場合は、[mainline](https://github.com/router-for-me/CLIProxyAPI) リポジトリに対して行ってください。
