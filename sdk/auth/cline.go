...
func (a *ClineAuthenticator) Login(ctx context.Context, cfg *config.Config, opts *LoginOptions) (*coreauth.Auth, error) {
	...
	ts := &cline.ClineTokenStorage{AccessToken: tokenResp.AccessToken, RefreshToken: tokenResp.RefreshToken, ExpiresAt: expiresAtInt, Email: email, Type: "cline"}
	fileName := cline.CredentialFileName(email)
	metadata := map[string]any{"email": email, "fileName": fileName, "expires_at": expiresAtInt}
	fmt.Printf("Cline authentication successful for %s\n", email)
	return &coreauth.Auth{ID: fileName, Provider: a.Provider(), FileName: fileName, Storage: ts, Metadata: metadata, Label: email}, nil
}
...
func waitForClineCallback(ctx context.Context, callbackPort int, prompt func(prompt string) (string, error)) (*clineOAuthResult, error) {
	...
	server := &http.Server{Addr: ":" + strconv.Itoa(callbackPort), Handler: mux, ReadHeaderTimeout: 5 * time.Second}
	mux.HandleFunc("/callback", func(w http.ResponseWriter, r *http.Request) {
		q := r.URL.Query()
		res := &clineOAuthResult{Code: strings.TrimSpace(q.Get("code")), State: strings.TrimSpace(q.Get("state")), Error: strings.TrimSpace(q.Get("error")), ErrorDescription: strings.TrimSpace(q.Get("error_description"))}
		select {
		case resultCh <- res:
		default:
		}
	...
	for {
		...
		case <-manualTimerC:
			...
			return &clineOAuthResult{Code: parsed.Code, State: parsed.State, Error: parsed.Error, ErrorDescription: parsed.ErrorDescription}, nil
		}
	}
}
